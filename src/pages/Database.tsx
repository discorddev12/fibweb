...
const fakeSubjects = [
  { name: "Marcus 'Ghost' Rivera", status: "WANTED", threat: "HIGH", lastSeen: "Vinewood Hills, Los Santos", charges: ["Armed Robbery", "Evading Law Enforcement", "██████████"], notes: "Subject is considered armed and dangerous. Last seen operating a black ████████ sedan. Known associates: ██████████, ██████████.", caseFile: "FIB-2024-00482" },
  { name: "Elena 'Ice' Vasquez", status: "UNDER SURVEILLANCE", threat: "MEDIUM", lastSeen: "Paleto Bay", charges: ["Money Laundering", "Conspiracy", "████████████"], notes: "Subject maintains a front business at ██████████ Ave. Intercepted communications suggest ties to ██████████ cartel operations.", caseFile: "FIB-2024-01197" },
  { name: "Derek 'Havoc' Thompson", status: "IN CUSTODY", threat: "LOW", lastSeen: "Bolingbroke Penitentiary", charges: ["Grand Theft Auto", "Assault on LEO"], notes: "Cooperating witness in Operation ██████████. Transferred to protective custody on ██/██/2024.", caseFile: "FIB-2024-00831" },
  { name: "████████ ██████", status: "CLASSIFIED", threat: "██████", lastSeen: "██████████████", charges: ["[REDACTED]", "[REDACTED]", "[REDACTED]"], notes: "██████████████████████████████████████████████████████████████████████████████████████████████████████.", caseFile: "FIB-████-█████" },
  { name: "Johnny 'Wrench' Kowalski", status: "WANTED", threat: "MEDIUM", lastSeen: "Sandy Shores", charges: ["Illegal Weapons Trafficking", "████████████"], notes: "Subject operates out of a scrapyard in ██████████. Surveillance ongoing since ██/██/2024. Linked to ██████████ MC.", caseFile: "FIB-2024-00295" },
];
...
export default function Database() {
  ...
  const fetchCases = useCallback(async () => {
    ...
    if (data) setDbCases(data as CaseRecord[]);
  }, []);
        ...
        <section className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="w-full max-w-md">
            <div className="bg-gradient-card border border-gold rounded-lg p-8 text-center space-y-6">
              ...
              <p className="text-muted-foreground/50 text-[10px] tracking-wider">UNAUTHORIZED ACCESS IS A FEDERAL OFFENSE — 18 U.S.C. § 1030</p>
            </div>
          </div>
        </section>
      </Layout>
    ...
    <Layout>
      <section className="py-20 px-4 border-b border-gold">
        <div className="container mx-auto max-w-5xl">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-primary font-serif tracking-wider">FIB FEDERAL DATABASE</h1>
          </div>
      ...
      <section className="py-4 px-4">
        <div className="container mx-auto max-w-3xl">
          <div className="relative">
            ...
            <input type="text" placeholder="Search subject name or case file number..."
          ...
          {!searched && <p className="text-muted-foreground/60 text-xs text-center mt-3 font-mono">Enter a query to search the federal database...</p>}
        </div>
      </section>
      ...
      {searched && (
        <section className="pb-10 px-4">
          <div className="container mx-auto max-w-3xl space-y-3">
            <p className="text-muted-foreground text-xs font-mono">{filteredSubjects.length} RECORD(S) FOUND</p>
              ...
              <div key={i} className="bg-gradient-card border border-gold rounded-lg p-5 space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {subject.charges.map((c, j) => (
                    <span key={j} className="bg-background border border-border rounded px-2 py-0.5 text-[10px] text-foreground/70 font-mono">{c}</span>
                  ))}
                </div>
                {subject.notes && (
                  <div className="bg-background/50 border border-border rounded p-3">
        ...
        <div className="container mx-auto max-w-3xl">
          ...
          <div className="space-y-2">
            {redactedFiles.map((file, i) => (
              <div key={i} className="bg-gradient-card border border-gold rounded-lg p-4 flex items-center justify-between gap-4">
                ...
                <span className="shrink-0 bg-red-900/40 text-red-300 border border-red-800 rounded px-2 py-0.5 text-[10px] font-bold tracking-wider">{file.classification}</span>
              </div>
            ))}
          </div>
          <div className="mt-6 bg-gradient-card border border-gold rounded-lg p-6 text-center">
            ...
            <p className="text-muted-foreground text-xs font-mono tracking-wider">
              NOTICE: ALL DATABASE ACTIVITY IS MONITORED AND LOGGED.<br />
